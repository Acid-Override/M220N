    ...
    public class UsersRepository
    {
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await _usersCollection.Find(new BsonDocument("email", email)).FirstOrDefaultAsync(cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                //PasswordHashOMatic.Hash(password)
                //
                // // user = new User...
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                //db.sessions.updateOne({"user_id" : user.Email}, {$set : { UserId : user.Email }, $set : { Jwt : user.AuthToken })
                               

                await _sessionsCollection.UpdateOneAsync(new BsonDocument("user_id", user.Email),
                    Builders<Session>.Update.Set(u => u.UserId, user.Email)
                    .Set(u => u.Jwt, user.AuthToken),
